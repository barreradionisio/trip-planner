      ...
      <>
        <style>{`
          * { box-sizing: border-box; }
          .cal-nav { background: #1667E6; padding: 12px 32px; display: flex; align-items: center; justify-content: space-between; position: relative; }
          .cal-nav-center { position: absolute; left: 50%; transform: translateX(-50%); display: flex; gap: 28px; align-items: center; }
          .cal-nav-mobile { display: none; }
          @media (max-width: 768px) {
            .cal-nav { padding: 12px 16px; }
            .cal-nav-center { display: none; }
            .cal-nav-mobile { display: flex; background: #1667E6; padding: 8px 16px; gap: 16px; border-top: 1px solid rgba(255,255,255,0.1); }
          }
        `}</style>
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Montserrat',sans-serif", background: "#f5f7ff", width: "100%", maxWidth: "100vw", overflow: "hidden" }}>
          <NavBar />
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
            <div style={{ background: "#fff", borderRadius: "16px", border: "1.5px solid #e8edf8", padding: "48px 32px", textAlign: "center", maxWidth: "480px", width: "100%" }}>
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .cal-stars { display: flex; gap: 4px; }
        .cal-recomienda { display: flex; gap: 12px; }
        .cal-footer {
          background: #0D0C56;
          padding: 32px 32px;
          display: flex;
        ...
        .cal-footer-links { display: flex; gap: 24px; }

        @media (max-width: 768px) {
          .cal-nav { padding: 12px 16px; }
          .cal-nav-center { display: none; }
          .cal-nav-mobile { display: flex; background: #1667E6; padding: 8px 16px; gap: 16px; border-top: 1px solid rgba(255,255,255,0.1); }
          .cal-hero { padding: 16px 16px 36px; }
          .cal-content { padding: 0 12px 24px; }
          .cal-aspecto { flex-wrap: wrap; }
          .cal-stars span { font-size: 22px !important; }
          .cal-recomienda { flex-direction: column; }
          .cal-footer { flex-direction: column; gap: 16px; padding: 24px 16px; align-items: flex-start; }
          .cal-footer-links { flex-direction: column; gap: 8px; }
        }
      `}</style>
        ...
        <div className="cal-content">
          ...
          <div style={{ background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", overflow: "hidden", marginBottom: "16px" }}>
            ...
            <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: "16px" }}>
              {aspectos.map(a => (
                <div key={a.key} className="cal-aspecto">
                  ...
                  <div className="cal-stars">
                    {[1, 2, 3, 4, 5].map(star => (
                      <span key={star} onClick={() => setCalificacion(a.key, star)} onMouseEnter={() => setHover(prev => ({ ...prev, [a.key]: star }))} onMouseLeave={() => setHover(prev => ({ ...prev, [a.key]: 0 }))} style={{ fontSize: "28px", cursor: "pointer", color: star <= (hover[a.key] || calificaciones[a.key] || 0) ? "#FF5C00" : "#e8edf8", transition: "color 0.1s" }}>★</span>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div style={{ background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", padding: "16px 18px", marginBottom: "16px" }}>
            ...
            <div className="cal-recomienda">
              <button onClick={() => setRecomendaria(true)} style={{ flex: 1, padding: "16px", border: `1.5px solid ${recomendaria === true ? "#3ED5A9" : "#e8edf8"}`, borderRadius: "10px", background: recomendaria === true ? "#e8fff5" : "#fff", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", fontFamily: "Montserrat, sans-serif" }}>
              ...
              <button onClick={() => setRecomendaria(false)} style={{ flex: 1, padding: "16px", border: `1.5px solid ${recomendaria === false ? "#E24B4A" : "#e8edf8"}`, borderRadius: "10px", background: recomendaria === false ? "#ffeaea" : "#fff", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "8px", fontFamily: "Montserrat, sans-serif" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", padding: "16px 18px", marginBottom: "16px" }}>
            ...
            <textarea value={comentario} onChange={e => setComentario(e.target.value)} placeholder="¿Qué fue lo mejor de tu viaje? ¿Hay algo que podríamos mejorar?" rows={4} style={{ width: "100%", border: "1.5px solid #e8edf8", borderRadius: "8px", padding: "10px 12px", fontSize: "12px", outline: "none", resize: "vertical", boxSizing: "border-box" as const, fontFamily: "'Montserrat',sans-serif", lineHeight: "1.6", color: "#0D0C56" }} />
            <div style={{ fontSize: "10px", color: "#aaa", marginTop: "4px", textAlign: "right" }}>{comentario.length}/500</div>
          </div>

          {/* BOTÓN */}
          <button onClick={enviar} disabled={!puedeEnviar} style={{ width: "100%", padding: "13px", backgroundColor: "#FF5C00", color: "#fff", border: "none", borderRadius: "13px", fontFamily: "sans-serif", fontWeight: "800", fontSize: "14px", cursor: !puedeEnviar ? "not-allowed" : "pointer", opacity: !puedeEnviar ? 0.4 : 1 }}>
            Enviar calificación
          </button>
          {!puedeEnviar && (
