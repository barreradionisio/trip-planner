...
const faqs = [
  {
    ...
    respuesta: "Sí, puedes editar tu itinerario desde Mis viajes en cualquier momento. Para cambios en vuelos u hoteles, contáctanos con al menos 72h de anticipación. Los cambios están sujetos a disponibilidad y pueden aplicarse cargos adicionales.",
  },
  {
    pregunta: "¿Cómo funciona la cancelación?",
    respuesta: "Las condiciones de cancelación dependen de la tarifa contratada en cada vuelo y hotel. Consulta los detalles de tu reserva en Mis viajes o escríbenos a soporte@tripplanner.com.mx.",
  },
  {
...
function SoporteContent() {
  ...
  const enviarMensaje = () => {
    ...
    setMensaje("");
    setTimeout(() => {
      setMensajes(prev => [...prev, { tipo: "agente", texto: "Gracias por tu mensaje. Un agente te responderá en breve. Si es urgente, escríbenos a soporte@tripplanner.com.mx" }]);
    }, 1000);
  };
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .sop-footer-links { display: flex; gap: 24px; }

        @media (max-width: 768px) {
          .sop-nav { padding: 12px 16px; }
          .sop-nav-center { display: none; }
          .sop-hero { padding: 28px 16px; }
          .sop-content { padding: 16px 12px 32px; }
          .sop-tabs { overflow-x: auto; -webkit-overflow-scrolling: touch; }
          .sop-contacto { flex-direction: column; align-items: flex-start; }
          .sop-footer { flex-direction: column; gap: 16px; padding: 24px 16px; align-items: flex-start; }
          .sop-footer-links { flex-direction: column; gap: 8px; }
        }
      `}</style>
      ...
      <div className="sop-wrap">
        ...
        <nav className="sop-nav">
          ...
          <div className="sop-nav-center">
            ...
            <Link href="/soporte" style={{ fontSize: "13px", color: "#1667E6", textDecoration: "none", fontWeight: "700", padding: "6px 14px", borderRadius: "50px", background: "#f0f5ff" }}>Soporte</Link>
          </div>
        ...
        <div className="sop-content">
          ...
          {/* TABS */}
          <div style={{ display: "flex", background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", padding: "4px", marginBottom: "20px", gap: "4px", overflowX: "auto" }}>
            {tabs.map(t => (
              <button key={t.key} onClick={() => setTab(t.key)} style={{ flex: 1, padding: "10px", border: "none", borderRadius: "10px", fontSize: "13px", fontWeight: "700", cursor: "pointer", background: tab === t.key ? "#1667E6" : "transparent", color: tab === t.key ? "#fff" : "#888", transition: "all 0.2s", whiteSpace: "nowrap", fontFamily: "Montserrat, sans-serif" }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {tab === "chat" && (
            <div style={{ background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", overflow: "hidden" }}>
              ...
              <div style={{ padding: "12px 16px", borderTop: "1px solid #f0f2fa", display: "flex", gap: "10px" }}>
                <input value={mensaje} onChange={e => setMensaje(e.target.value)} onKeyDown={e => e.key === "Enter" && enviarMensaje()} placeholder="Escribe tu mensaje..." style={{ flex: 1, border: "1.5px solid #e8edf8", borderRadius: "8px", padding: "9px 12px", fontSize: "12px", outline: "none", minWidth: 0 }} />
                <button onClick={enviarMensaje} style={{ padding: "9px 18px", background: "#1667E6", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "700", fontSize: "12px", cursor: "pointer", fontFamily: "Montserrat, sans-serif", flexShrink: 0 }}>Enviar</button>
              </div>
            </div>
          )}
          ...
          {tab === "faq" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {faqs.map((f, i) => (
                <div key={i} style={{ background: "#fff", borderRadius: "13px", border: `1.5px solid ${faqAbierto === i ? "#1667E6" : "#e8edf8"}`, overflow: "hidden" }}>
                  <button onClick={() => setFaqAbierto(faqAbierto === i ? null : i)} style={{ width: "100%", padding: "14px 18px", border: "none", background: "transparent", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", textAlign: "left", fontFamily: "Montserrat, sans-serif" }}>
                    <span style={{ fontWeight: "700", fontSize: "13px", color: "#0D0C56", textAlign: "left" }}>{f.pregunta}</span>
              ...
              {!ticketEnviado ? (
                <>
                  ...
                  <div style={{ padding: "14px 18px", borderTop: "1px solid #f0f2fa", display: "flex", justifyContent: "flex-end" }}>
                    <button onClick={enviarTicket} style={{ padding: "10px 24px", backgroundColor: "#FF5C00", color: "#fff", border: "none", borderRadius: "8px", fontWeight: "700", fontSize: "12px", cursor: "pointer", fontFamily: "Montserrat, sans-serif" }}>Enviar ticket</button>
                  </div>
                </>
              ) : (
          ...
          {/* CONTACTO DIRECTO */}
          <div className="sop-contacto">
            <div>
              <div style={{ fontWeight: "700", fontSize: "13px", color: "#0D0C56", marginBottom: "2px" }}>¿Prefieres escribirnos directamente?</div>
              <div style={{ fontSize: "12px", color: "#888" }}>soporte@tripplanner.com.mx · Respuesta en menos de 24h</div>
            </div>
            <a href="mailto:soporte@tripplanner.com.mx" style={{ padding: "9px 18px", background: "#f0f5ff", color: "#1667E6", border: "1.5px solid #e0eaff", borderRadius: "8px", fontWeight: "700", fontSize: "12px", textDecoration: "none", whiteSpace: "nowrap" }}>Escribir correo</a>
          </div>
        </div>
        ...
        <footer className="sop-footer">
          ...
          <div className="sop-footer-links">
