...
export default function Hospedaje() {
  ...
  const hotelesFiltrados = ciudades[ciudadActiva].hoteles
    ...
    .sort((a, b) => ordenar === "precio" ? a.precio - b.precio : b.estrellas - a.estrellas);
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .hosp-pasos { display: flex; align-items: center; gap: 4px; }
        .hosp-paso-mobile { display: none; font-size: 11px; font-weight: 700; color: #fff; white-space: nowrap; align-items: center; gap: 8px; }
        .hosp-body {
          display: grid;
        ...
        .hosp-sidebar { display: flex; flex-direction: column; }
        .hosp-sidebar-mobile { display: none; flex-direction: column; width: 100%; }
        .hosp-hab-grid {
          display: grid;
        ...
        .hosp-card:active { opacity: 0.8; }

        @media (max-width: 768px) {
          .hosp-pasos { display: none; }
          .hosp-paso-mobile { display: flex; }
          .hosp-body { grid-template-columns: 1fr; padding: 12px; overflow: hidden; }
          .hosp-sidebar { display: none; }
          .hosp-sidebar-mobile { display: flex; }
          .hosp-hab-grid { grid-template-columns: 1fr 1fr; }
          .hosp-card { max-width: 100%; }
        }
      `}</style>
      ...
      <div className="hosp-wrap">
        ...
        <div className="hosp-topbar">
          ...
          <div className="hosp-paso-mobile">
        ...
        <div className="hosp-body">
          ...
          <div style={{ overflow: "hidden", width: "100%" }}>
                ...
                {tiposHabitacion.map(t => (
                  <div key={t.key} style={{ border: `1.5px solid ${habitaciones[t.key] > 0 ? "#1667E6" : "#e8edf8"}`, borderRadius: "10px", padding: "10px 8px", background: habitaciones[t.key] > 0 ? "#f0f5ff" : "#fff", textAlign: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
                      <button onClick={() => actualizarHab(t.key, -1)} style={{ width: "22px", height: "22px", borderRadius: "50%", border: "1.5px solid #e8edf8", background: "#fff", cursor: "pointer", fontSize: "14px", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Montserrat, sans-serif" }}>−</button>
                    ...
                    <div style={{ marginBottom: "14px" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                        {[{ v: 0, l: "Todos" }, { v: 3, l: "3★+" }, { v: 4, l: "4★+" }, { v: 5, l: "5★+" }].map(e => (
                          <button key={e.v} onClick={() => setFiltroEstrellas(e.v)} style={{ padding: "5px 12px", border: `1.5px solid ${filtroEstrellas === e.v ? "#1667E6" : "#e8edf8"}`, borderRadius: "50px", fontSize: "11px", fontWeight: "600", cursor: "pointer", background: filtroEstrellas === e.v ? "#1667E6" : "#fff", color: filtroEstrellas === e.v ? "#fff" : "#888", fontFamily: "Montserrat, sans-serif" }}>
                            {e.l}
                          </button>
                        ))}
                      </div>
                    ...
                    <div style={{ marginBottom: "14px" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                        {serviciosDisponibles.map(s => (
                          <button key={s} onClick={() => toggleServicio(s)} style={{ padding: "5px 12px", border: `1.5px solid ${serviciosSeleccionados.includes(s) ? "#1667E6" : "#e8edf8"}`, borderRadius: "50px", fontSize: "11px", fontWeight: "600", cursor: "pointer", background: serviciosSeleccionados.includes(s) ? "#1667E6" : "#fff", color: serviciosSeleccionados.includes(s) ? "#fff" : "#888", fontFamily: "Montserrat, sans-serif" }}>
                            {s}
                          </button>
                        ))}
                      </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {hotelesFiltrados.map(h => (
                <button key={h.id} className="hosp-card" onClick={() => seleccionar(ciudades[ciudadActiva].ciudad, h.id)}
                  style={{ background: seleccionados[ciudades[ciudadActiva].ciudad] === h.id ? "#f8faff" : "#fff", border: `1.5px solid ${seleccionados[ciudades[ciudadActiva].ciudad] === h.id ? "#1667E6" : "#e8edf8"}` }}>
          ...
          <div className="hosp-sidebar">
            ...
            <Link href={puedeContinuar ? "/itinerario" : "#"} style={{ width: "100%", padding: "13px", backgroundColor: "#FF5C00", color: "#fff", borderRadius: "13px", fontFamily: "sans-serif", fontWeight: "800", fontSize: "14px", opacity: !puedeContinuar ? 0.4 : 1, textDecoration: "none", display: "block", textAlign: "center" }}>
              Continuar → Itinerario
            </Link>
            {!puedeContinuar && <div style={{ fontSize: "11px", color: "#888", textAlign: "center", marginTop: "8px" }}>Selecciona hotel para cada ciudad y al menos una habitación</div>}
          </div>

          {/* SIDEBAR MÓVIL */}
          <div className="hosp-sidebar-mobile">
            <div style={{ background: "#fff", borderRadius: "13px", border: "1.5px solid #e8edf8", overflow: "hidden", marginBottom: "12px" }}>
              <div style={{ padding: "10px 14px", background: "#0D0C56" }}>
                <div style={{ fontFamily: "sans-serif", fontWeight: "800", fontSize: "12px", color: "#fff" }}>Hospedaje seleccionado</div>
              </div>
              <div style={{ padding: "12px 14px" }}>
                {ciudades.map(c => {
                  ...
                  return (
                    <div key={c.ciudad} style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", padding: "5px 0", borderBottom: "1px solid #f5f7ff" }}>
                      <span style={{ color: "#888" }}>{c.ciudad}</span>
                      <span style={{ fontWeight: "600", color: hotel ? "#1667E6" : "#aaa" }}>{hotel ? `$${hotel.precio}/noche` : "Pendiente"}</span>
                    </div>
                  );
                })}
              </div>
